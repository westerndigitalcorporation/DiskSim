...
dm_layout_g1_loadparams(struct lp_block *b, struct dm_disk_if *d)
{
    ...
    result->hdr = g1_layout_tracksparing;   // hurst_r hard code for 64 bit
...
//*****************************************************************************
// Function: getdefects (hurst_r done)
//    Parses the lp_list from the parser that contains LBA slip entries and
//    populates the G1 format data structure.
//
// Parameters:
//   struct dm_layout_g1_band *     pointer to data structure to populate
//   struct lp_list *               pointer to parser data structure containing slip entries
//
// Returns: int
//   -1 on error, otherwise 0
//*****************************************************************************

int getslips(struct dm_layout_g1_band *z, struct lp_list *b)
{
  int c; LBA_t *i;
  z->numslips = 0;
...
#ifdef DEBUG_LAYOUT_G1_LOAD
//  dump_lp_list( b, "getslips" );
#endif

  for(c = 0; c < b->values_len; c++) {
      if(b->values[c]) {
          if (b->values[c]->t != TYPE_I) {
              fprintf(stderr, "*** error: Zone slips must be INT64s.\n");
              return -1;
          }
  ...
  i = z->slip = (LBA_t *)malloc(z->numslips * sizeof(LBA_t));
...
//*****************************************************************************
// Function: getdefects (hurst_r done)
//    Parses the lp_list from the parser that contains LBA defect entries and
//    populates the G1 format data structure.
//
// Parameters:
//   struct dm_layout_g1_band *     pointer to data structure to populate
//   struct lp_list *               pointer to parser data structure containing defect entries
//
// Returns: int
//   -1 on error, otherwise 0
//*****************************************************************************

int getdefects(struct dm_layout_g1_band *z, struct lp_list *b)
{
    int c;
    LBA_t *i, *j;
    ...
    for(c = 0; c < b->values_len; c++) {
        if(b->values[c]) {
            if (b->values[c]->t != TYPE_I) {
                fprintf(stderr, "*** error: Zone defects must be INT64s.\n");
                return -1;
            }
            else if (b->values[c]->v.i < 0) {
    ...
    i = z->defect = (LBA_t *)calloc(z->numdefects * sizeof(LBA_t), 1);
    j = z->remap  = (LBA_t *)calloc(z->numdefects * sizeof(LBA_t), 1);
