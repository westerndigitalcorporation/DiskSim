        ...
        struct dm_mech_acctimes *breakdown)
{
  ...
  if(immed) {
    int firstsect = d->mech->dm_access_block( d, &state, start->sector, len, immed);
        ...
        struct dm_mech_acctimes *breakdown)
{
    ...
    LBA_t lbnhigh; // highest lbn on the current track

    LBA_t l;       // first lbn to read
    LBA_t lastlbn; // last lbn to read
    ...
    d->layout->dm_translate_ptol(d, &pbn, &l, NULL );
    ...
    do {
        ...
        d->layout->dm_get_track_boundaries(
                d,
                &pbn,
                (LBA_t)0,        // first lbn on track
                &lbnhigh,
                (LBA_t *)0);       // remapsector

        ddbg_assert((lbnhigh != DM_SLIPPED) && (lbnhigh != DM_REMAPPED));

        if(lbnhigh > lastlbn) { lbnhigh = lastlbn; }
