...
static dm_ptol_result_t
ptol(struct dm_disk_if *d, struct dm_pbn *p, LBA_t *lbn, LBA_t *remapsector)
{
//  int i;
...
static dm_ptol_result_t
ptol_0t(struct dm_disk_if *d, struct dm_pbn *p, LBA_t *remapsector)
{
  ddbg_assert(0);
  return 0;
}
...
static int
st_lbn(struct dm_disk_if *d, LBA_t lbn)
{
  struct dm_layout_g2_zone *z = find_zone_lbn(d, lbn);
  return z->st;
}
        ...
        LBA_t *remapsector)
{
    ...
    if(l1) {
        do {
            ...
            result = d->layout->dm_translate_ptol(d, &p1, l1, remapsector);
            p1.sector++;
        } while((result == DM_NX) && (p1.sector < z->st));
    }
    ...
    if(l2) {
        do {
            ...
            result = d->layout->dm_translate_ptol(d, &p2, l2, remapsector);
        } while((result == DM_NX) && p2.sector);
    }
