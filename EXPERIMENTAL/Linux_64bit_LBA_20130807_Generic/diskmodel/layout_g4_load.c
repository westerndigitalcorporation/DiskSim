...
marshal_layout_g4(struct dm_layout_g4 *lin) {
  ...
  dump_g4_layout( lin, "marshal_layout_g4" );
#endif
...
#ifdef DEBUG_MODEL_G4
  dump_g4_layout( r, "g4_load_slips" );
#endif
	       ...
	       struct lp_list *l)
{
  ...
  #ifdef DEBUG_MODEL_G4
  dump_g4_layout( r, "g4_load_remaps" );
#endif
...
void dump_g4_layout( struct dm_layout_g4 * g4l, char *name )
{
    ...
    for( j = 0; j < g4l->idx_len; j ++ )
    {
        ...
        for( i = 0; i < pIdx->ents_len; i++ )
        {
            ...
            {
                fprintf( stderr, "\n      TRACK: lbn %lld, cyl %d, off %d, len %lld, cyllen %d, alen %d, runlen %lld, cylrunlen %d, childtype %d, head %d"
            ...
            {
                fprintf( stderr, "\n      INDEX: lbn %lld, cyl %d, off %d, len %lld, cyllen %d, alen %d, runlen %lld, cylrunlen %d, childtype %d"
