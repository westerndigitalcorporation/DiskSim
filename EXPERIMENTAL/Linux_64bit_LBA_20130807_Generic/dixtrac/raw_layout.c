...
/* COUNT_THRSHLD is obsolete */
#define COUNT_THRSHLD  40
// #define COUNT_THRSHLD  19  /* this one was for Barracuda 181 or Fujitsu */
// #define COUNT_THRSHLD  50 /* this one is for the Cheetah73 */

/* the threshold divides the total number of tracks */
#define TRK_THRSHLD  80

/* the difference (number of cylinders) to distinguish different zones */
#define PERCENT        0.04
...
// #define MIN_CYL_DISTANCE 3  /* this one is for the Fujitsu disk */

#define MAX_DISTANCES 2

static int cyldist_index = 0;
static int cyl_distances[] = {20,3,0};
static int min_cyl_distance;
 ...
 *---------------------------------------------------------------------------*/
static int
determine_zones(disk *currdisk, sfreq_t *sectfreq, int capacity,int numtracks)
{
  typedef struct sp {
    int spt;
    int count;
  ...
  spt_t *sectspertrk, *currspt, *temp, *prev;
  int i;
  int mincyl = 10000;
  int maxcyl = 0;
  ...
  for(i=1;i<MAXSECTORS;i++) {
	...
	if (abs(avg - currspt->avgcyl) > (currspt->avgcyl * PERCENT)) {
	  ...
	  if (sectfreq[i].count > (numtracks / TRK_THRSHLD)) {
...
#endif
      /* this is to get a phony zone info */
      if (mincyl > currdisk->bands[i].startcyl) 
	mincyl = currdisk->bands[i].startcyl;
      if (maxcyl < currdisk->bands[i].endcyl) 
	maxcyl = currdisk->bands[i].endcyl;
...
#if 0
  if (total != currdisk->numblocks) {
    /* create a phony zone */
    /*  currdisk->numbands++; */
    currdisk->bands[i].blkspertrack = 0;
    currdisk->bands[i].startcyl = mincyl;
    currdisk->bands[i].endcyl = maxcyl;
    currdisk->bands[i].blksinband = currdisk->numblocks;
#ifdef PRINT_LAYOUT_INFO
    printf("\n *** Added PHONY zone ***\n");
    printf("%3d\t%8d\t%8d",currdisk->bands[i].blkspertrack,
	   currdisk->rlcyls[currdisk->bands[i].startcyl].firstlbn,
...
#ifdef PRINT_LAYOUT_INFO
   printf("The total number of tracks = %d\n",numtracks);
#endif
   i--;
   return((currdisk->rlcyls[currdisk->bands[i].startcyl].firstlbn +
	   currdisk->bands[i].blksinband - 1) == (total-1));
}
