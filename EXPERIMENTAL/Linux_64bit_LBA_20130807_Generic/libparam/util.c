...
void dump_lp_param( const struct lp_param *p, char *name );
...
struct lp_value *lp_new_intv( int64_t i)
{
...
int lp_inst_list(struct lp_inst *i)
{
  ...
  for(c = 0; c < i->l->values_len; c++) {
    ...
    ddbg_assert3(i->l->values[c]->t == TYPE_S,
...
#ifdef DEBUG_UTIL
//            if( 25 == b->type)
            dump_lp_param( b->params[c], "check_types" );
#endif
            if(lp_param_name(b->type, b->params[c]->name) == -1) {
                ...
                if((lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type == TYPE_D) && (PTYPE(b->params[c]) == TYPE_I)) {
                    b->params[c]->v->t = TYPE_D;
            ...
            if(PTYPE(b->params[c]) >= TYPE_BLOCK) {
                check_types(BVAL(b->params[c]));
            }
            else if(PTYPE(b->params[c]) == TYPE_LIST) {
                int d;
                struct lp_list *l = LVAL(b->params[c]);
                for(d = 0; d < l->values_len; d++) {
                    if(l->values[d]) {
                        if(l->values[d]->t >= TYPE_BLOCK) {
  ...
  for(c = 0; c < overrides_len; c += 3) {
	...
	for(d = 0; d < spec_copy->params_len; d++) {
	  ...
	  if(!strcmp(p1, spec_copy->params[d]->name)) {
	    if(spec_copy->params[d]->v->t != TYPE_BLOCK) {
...
{
    fprintf( stderr, "\nlp_param: %s", name );
    fprintf( stderr, "\n   source file: %s", p->source_file );
    fprintf( stderr, "\n   name       : %s", p->name );
    fprintf( stderr, "\n   lp_file    : %s", p->v->source_file );
    fprintf( stderr, "\n   lp_type    : %d (%s)", p->v->t, lp_builtin_names[ abs((int64_t)p->v->t) ] );
    switch( p->v->t )
    {
      case TYPE_I:
        fprintf( stderr, "\n   lp_value   : %lld", p->v->v.i );
        break;
      case TYPE_D:
        fprintf( stderr, "\n   lp_value   : %f", p->v->v.d );
        break;
...
void dump_lp_block( const struct lp_block *b, char *name )
{
    ...
    for( paramsIndex = 0; paramsIndex < b->params_len; paramsIndex++ )
    {
        ...
        if( NULL == param )
            break;
        dump_lp_param( b->params[paramsIndex], name );
    }
    fflush( stderr );
}
#endif // DEBUG_UTIL
