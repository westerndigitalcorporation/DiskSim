...
struct device_header *getdevbyname(
                        char *name,
                        int *gdevnum, /* unique for all devices */
                        int *ldevnum, /* unique for all devices of same type */
                        int *type);
...
struct device_header {
  ...
  LBA_t   (*get_number_of_blocks)(int diskno);
  ...
  void    (*bus_delay_complete)(
              int devno,
              ioreq_event *curr,
              int sentbusno);
  
  void    (*bus_ownership_grant)(
              int devno,
              ioreq_event *curr,
              int busno,
              double arbdelay);
...
LBA_t   device_get_number_of_blocks (int diskno);
...
int     device_get_distance (
            int diskno,
            ioreq_event *req,
            int exact,
            int direction);

double  device_get_servtime (
            int diskno,
            ioreq_event *req,
            int checkcache,
            double maxtime);
...
double  device_get_acctime (
            int diskno,
            ioreq_event *req,
            double maxtime);

void    device_bus_delay_complete (
            int devno,
            ioreq_event *curr,
            int sentbusno);

void    device_bus_ownership_grant (
            int devno,
            ioreq_event *curr,
            int busno,
            double arbdelay);
