...
void dump_disk_buffer_seqments ( segment *seg, const char *msg )
{
    ...
    fprintf( outputfile, "\ndump_disk_buffer_seqments (%s): %p", msg, seg );
    fprintf( outputfile, "\nindex,seg,time,state,startblkno,endblkno,outstate,outbcount,minreadaheadblkno,maxreadaheadblkno,size,hold_blkno,hold_bcount" );

    segment *nextSeg = seg;
    for( i = 0; NULL != nextSeg; i++)
    {
        fprintf( outputfile, "\n%d,%p,%f,%d,%lld,%lld,%d,%d,%lld,%lld,%d,%lld, %d",
                i,
                nextSeg,
                nextSeg->time,
                nextSeg->state,
                ...
                nextSeg->outstate,
                nextSeg->outbcount,
                nextSeg->minreadaheadblkno,
                nextSeg->maxreadaheadblkno,
                nextSeg->size,
                nextSeg->hold_blkno,
                nextSeg->hold_bcount
        );
...
void disk_buffer_segment_wrap (segment *seg, LBA_t endblkno)
{
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_segment_wrap  seg %p, endblkno %lld\n", simtime, seg, endblkno );
    fflush( outputfile );
#endif
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_overlap  seg %p, devno %d, blkno %lld, bcount %d, flags %X\n", simtime, seg, curr->devno, curr->blkno, curr->bcount, curr->flags );
    fflush( outputfile );
#endif
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_remove_from_seg  currdiskreq %p\n", simtime, currdiskreq );
    fflush( outputfile );
#endif
...
int disk_buffer_block_available (disk *currdisk, segment *seg, LBA_t blkno)
{
    diskreq *seg_owner;

#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_block_available  currdisk %p, seg %p, blkno %lld\n", simtime, currdisk, seg, blkno );
    fflush( outputfile );
#endif
    ...
    if (seg->state == BUFFER_READING) {
        ...
        if ((blkno < seg->startblkno) && ((!currdisk->readanyfreeblocks) || (seg->endblkno != seg->startblkno))) {
            return(FALSE);
        }
...
static void disk_buffer_stats (disk *currdisk, ioreq_event *curr, segment *seg, int hittype)
{
    ...
    default:
        fprintf(stderr, "Invalid hittype in disk_buffer_stats - blkno %lld, bcount %d, state %d\n", curr->blkno, curr->bcount, seg->state);
        exit(1);
    }
    return;
}
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_select_read_segment  currdisk %p, currdiskreq %p\n", simtime, currdisk, currdiskreq );
    fflush( outputfile );
#endif
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "%f: disk_buffer_select_read_segment  segment = %8p, hittype = %d\n",simtime, currdiskreq->seg,currdiskreq->hittype);
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_select_segment  currdisk %p, currdiskreq %p, set_extradisc %d\n", simtime, currdisk, currdiskreq, set_extradisc );
    fflush( outputfile );
#endif
...
void disk_buffer_set_segment (disk *currdisk, diskreq *currdiskreq)
{
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_set_segment  currdisk %p, currdiskreq %p\n", simtime, currdisk, currdiskreq );
    dump_disk_buffer_seqments ( currdisk->seglist, "main cache" );
    dump_disk_buffer_seqments ( currdisk->dedicatedwriteseg, "write cache" );
    fflush( outputfile );
#endif
    ...
    default:
        fprintf(stderr, "Invalid segment state in disk_buffer_set_segment - blkno %lld, bcount %d, state %d\n", currdiskreq->ioreqlist->blkno, currdiskreq->ioreqlist->bcount, seg->state);
        exit(1);
    }
...
#ifdef DEBUG_DISKCACHE
    fprintf (outputfile, "*** %f: disk_buffer_attempt_seg_ownership  currdisk %p, currdiskreq %p, seg %p\n", simtime, currdisk, currdiskreq, seg );
    fflush( outputfile );
#endif
