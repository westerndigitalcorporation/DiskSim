...
static void ioqueue_print_subqueue_state (subqueue *queue)
{
    ...
    if( queue->listlen > 0 )
    {
        ...
        fprintf(outputfile, "*** %f:     Subqueue state: lastblkno %lld, lastcylno %d, lastsurface %d, dir %d\n", simtime, queue->lastblkno, queue->lastcylno, queue->lastsurface, queue->dir);
        for (i = 0; i < queue->iobufcnt; i++) {
            fprintf(outputfile, "*** %f:     depth %d, state %d, devno %d, blkno: %lld, bcount %d, flags %x, cylno %d, surface %d\n", simtime, i, tmp->state, tmp->devno, tmp->blkno, tmp->totalsize, tmp->flags, tmp->cylinder, tmp->surface);
            tmp = tmp->next;
        }
        //       fprintf( outputfile, "\n" );
    }
...
{
    fprintf (outputfile, "*** %f: ioqueue_print_contents: queue %p\n", simtime, queue );
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_insert_new_request  subqueue %p, sched_alg = %d, devno %d, blkno %lld, bcount %d, flags %x\n", simtime, queue, queue->sched_alg, temp->devno, temp->blkno, temp->totalsize, temp->flags );
    fflush( outputfile );
#endif
   ...
   weight = (double) queue->bigqueue->to_time;*/
    if(sched_count != 0){
        if(requests[current_head] != NULL){
            ...
            current_head++;
            sched_count--;
            fprintf (stderr, "2Selected request: %f, cylno %d, blkno %lld, read %d, devno %d\n",
    ...
    if(sched_count != 0){
        if(requests[current_head] != NULL){
            ...
            fprintf (stderr, "3Selected request: %f, cylno %d, blkno %lld, read %d, devno %d %f\n",
...
#ifdef DEBUG_IOQUEUE
        fprintf (outputfile, "*** %f: ioqueue_get_next_request_from_subqueue  Selected request: blkno %lld, cylno %d, surface %d\n", simtime, temp->blkno, temp->cylinder, temp->surface );
        fflush(outputfile);
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_request_match  blkno %lld\n", simtime, curr->blkno );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_get_specific_request_from_subqueue  blkno %lld\n", simtime, wanted->blkno );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_set_starttime_in_subqueue  blkno %lld\n", simtime, target->blkno );
    fflush(outputfile);
#endif
...
static ioreq_event * ioqueue_remove_completed_request (subqueue *queue, ioreq_event *done)
{
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_remove_completed_request  listlen %d, starttime %f, stoptime %f, devno %d, blkno %lld, bcount %d, flags 0x%x\n", simtime, queue->listlen, tmp->starttime, tmp->iob_un.time, tmp->devno, tmp->blkno, tmp->totalsize, tmp->flags );
#endif
    ...
    if ((tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) {
        ...
        if ((tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) {
            fprintf(stderr, "Completed event not found pending in ioqueue - blkno %lld, tmp->blkno %lld, tmp->totalsize %d, %d, %d, %d, %d\n", done->blkno, tmp->blkno, tmp->totalsize, (tmp == NULL), done->opid, tmp->opid, done->bcount);
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_get_specific_request  blkno %lld\n", simtime, wanted->blkno );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: ioqueue_set_starttime  blkno %lld\n", simtime, target->blkno );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf (outputfile, "*** %f: Entering ioqueue_add_new_request: type %d, devno %d, blkno %lld, bcount %d, flags 0x%x\n", simtime, new_event->type, new_event->devno, new_event->blkno, new_event->bcount, new_event->flags );
#endif
...
#ifdef DEBUG_IOQUEUE
    fprintf(outputfile, "Entering ioqueue_physical_access_done: devno %d, blkno %lld, bcount %d, flags %x\n", curr->devno, curr->blkno, curr->bcount, curr->flags);
