...
static int logorg_overlap (logorg *currlogorg, int devno, ioreq_event *curr, LBA_t blksperpart)
{
   LBA_t calc;
...
*/
   ...
   if (currlogorg->stat.outstanding >= MAX_QUEUE_LENGTH) {
      fprintf(stderr,"logorg_maprequest_update_stats: Stopping simulation because of saturation: simtime %f, totalreqs %d\n", simtime, disksim->totalreqs);
      fflush(stderr);
      fprintf (outputfile, "logorg_maprequest_update_stats: Stopping simulation because of saturation: simtime %f, totalreqs %d, MAX_QUEUE_LENGTH %d\n", simtime, disksim->totalreqs, MAX_QUEUE_LENGTH);
      fprintf (outputfile, "last request:  dev=%d, blk=%lld, cnt=%d, %d (R==1)\n",curr->devno, curr->blkno, curr->bcount, (curr->flags & READ));
      disksim_simstop();
   }
   if (req->flags & READ) {
...
fprintf (outputfile, "flags %x, opid %d\n", curr->flags, curr->opid);
    ...
    if (logorgno == -1) {
        fprintf (stderr, "unexpected request location: devno %x, blkno %lld, bcount %d\n", curr->devno, curr->blkno, curr->bcount);
    }
    ASSERT(logorgno != -1);
...
void logorg_raise_priority (logorg **logorgs, int numlogorgs, int opid, int devno, LBA_t blkno, void *buf)
{
   int i, j;
   int logorgno = -1;
   LBA_t calc;
...
/*     currlogorg->devs = &devs[0]; */
   ...
   if ((currlogorg->stripeunit < 0) ||
      ...
      fprintf(stderr, "Invalid value for 'Stripe unit (in sectors)': %lld (<0 || >%lld)", currlogorg->stripeunit, currlogorg->blksperpart);
   }

   if ((currlogorg->parityunit < 0) ||
       (currlogorg->parityunit > currlogorg->blksperpart)) {
      fprintf(stderr, "Invalid value for 'Parity stripe unit': %lld", currlogorg->stripeunit);
   }
   ...
   if ((currlogorg->stripeunit < 0) ||
       (currlogorg->stripeunit > currlogorg->blksperpart)) {
      fprintf(stderr, "Invalid value for stripeunit at logorg_postpass_per: %lld\n", currlogorg->stripeunit);
      exit(1);
   }
