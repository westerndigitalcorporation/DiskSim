...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_send_event_up_path - devno %d, type %d, cause %d, blkno %lld, bcount %d\n", simtime, curr->devno, curr->type, curr->cause, curr->blkno, curr->bcount );
    fflush( outputfile );
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_bus_delay_complete - devno %d, type %d, cause %d, blkno %lld, bcount %d\n", simtime, curr->devno, curr->type, curr->cause, curr->blkno, curr->bcount );
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_request_complete\n", simtime );
    fflush( outputfile );
#endif
...
static void disk_reconnection_or_transfer_complete (ioreq_event *curr)
{
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_reconnection_or_transfer_complete - currdiskreq %p, devno %d, type %d, cause %d, blkno %lld, bcount %d\n", simtime, currdiskreq, curr->devno, curr->type, curr->cause, curr->blkno, curr->bcount );
    fflush(outputfile);
#endif
    ...
    fprintf (outputfile, "********** %f: disk_reconnection_or_transfer_complete - type %s (%d(, devno %d, blkno %lld, bcount %d, flags %X\n", simtime, getEventString( curr->type ), curr->type, curr->devno, curr->blkno, curr->bcount, curr->flags );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_release_hda\n", simtime );
    fflush(outputfile);
#endif
        ...
        int check_watermark)
{
    ...
    segment     *seg = currdiskreq->seg;
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_request_needs_bus\n", simtime );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_check_bus\n", simtime );
    fflush(outputfile);
#endif
    disksim_inst_enter();

#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "%f  %8p  Entering disk_check_bus for disk %d\n", simtime, currdiskreq,currdisk->devno);
    fflush(outputfile);
#endif
        ...
        double mintime)
{
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "\n%f: disk_initiate_seek to devno %d, blkno %lld, bcount %d, flags %x\n", simtime, curr->devno, curr->blkno, curr->bcount, curr->flags);
    fflush(outputfile);
#endif
    ...
    if( NULL != DISKSIM_DISK_LOG )
    {
        ...
        fprintf( DISKSIM_DISK_LOG, "\n%f %d %lld %d %d (%x)   currCyl %d, currHead %d, currAngle %u, rotationTimeMSec %f, realAngle %f, destCyl %d, destHead %d, destSector %d, seekdist %d, seektime %f",
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_get_effectivehda\n", simtime );
    fflush(outputfile);
#endif
...
{
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_check_hda_pending\n", simtime );
    fflush(outputfile);
#endif
    disksim_inst_enter();
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_check_hda\n", simtime );
    fflush(outputfile);
#endif
    disksim_inst_enter();
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_activate_read\n", simtime );
    fflush(outputfile);
#endif
    disksim_inst_enter();
...
#ifdef DEBUG_DISKCTLR
    fprintf(outputfile, "%f: disk_request_arrive  type=%d, ", simtime, curr->type );
    fprintf(outputfile, "cause %d, devno %d, blkno %lld, bcount %d, flags 0x%x\n", curr->cause, curr->devno, curr->blkno, curr->bcount, curr->flags );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_completion\n", simtime );
    fflush(outputfile);
#endif
...
{
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_interrupt_complete\n", simtime );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "%f: disk_event_arrive  Entering  type %s (%d), devno %d, blkno %lld, bcount %d\n", simtime, getEventString(curr->type), curr->type, curr->devno, curr->blkno, curr->bcount );
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "*** %f: disk_event_arrive  Exiting\n", simtime );
#endif
}
...
disk_buffer_update_outbuffer(disk *currdisk, segment *seg)
{
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "%f: disk_buffer_update_outbuffer  inblkno %lld, startblkno %lld, endblkno %lld, devno %d, blkno %lld, bcount %d, flags %x\n", simtime, seg_owner->inblkno, seg->startblkno, seg->endblkno, seg->access->devno, seg->access->blkno, seg->access->bcount, seg->access->flags );
    fflush( outputfile );
#endif
    ...
    if((seg->state == BUFFER_WRITING)
            && ((seg_owner->inblkno < seg->startblkno) || (seg_owner->inblkno > seg->endblkno)))
    {
        ddbg_assert3(0, ("Error with inblkno at disk_buffer_update_outbuffer: "
                "inblkno %lld, startblkno %lld, endblkno %lld, blks %d\n",
                seg_owner->inblkno,
                seg->startblkno,
                seg->endblkno,
                blks));
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_buffer_transfer_size  currdiskreq %p, devno %d, blkno %lld, bcount %d, flags %X\n", simtime, currdiskreq, curr->devno, curr->blkno, curr->bcount, curr->flags );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_enablement_function\n", simtime );
    fflush(outputfile);
#endif
...
disk_buffer_seekdone(disk *currdisk, ioreq_event *curr)
{
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "%f: [%s] - Entering type %d, cause %d, devno %d, blkno %lld, bcount %d\n", simtime, __func__, curr->type, curr->cause, curr->devno, curr->blkno, curr->bcount );
#endif
...

    {
        ...
        // get lowest and highest LBN on track
    ...
    // At the very minimum, slips will screw this up.
    {
        ...
        // hurst_r the following code is a type error, different from base code
        dm_ptol_result_t result = currdisk->model->layout->dm_translate_ptol(currdisk->model, &tmppbn, &tmpblkno, 0);
        ddbg_assert(result == DM_OK);
    }

    // check if the lbn is within the track boundary
        ...
        LBA_t *remapsector)
{
    ...
    *next = *last+1;
    ...
    rv = cd->model->layout->dm_translate_ltop(cd->model, *blkno, MAP_FULL, &pbn, remapsector);
    ddbg_assert(rv == DM_OK);
...
disk_buffer_sector_done (disk *currdisk, ioreq_event *curr)
{
    ...
    LBA_t first, last;    // first/last lbn on track
    LBA_t next; // first lbn on next track
    LBA_t firstblkno;
    LBA_t lastblkno;
    LBA_t lbn = 0LL;
    int endlat = 0;
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "%f: disk_buffer_sector_done - Entering  type %d, cause %d, devno %d, blkno %lld, bcount %d\n", simtime, curr->type, curr->cause, curr->devno, curr->blkno, curr->bcount );
    fflush( outputfile );
#endif
    ...
    if(endlat == 2) {
        ...
        currdisk->stat.xfertime = (double) 0.0;
#ifdef DISKLOG_ON
    // this conditional concatenates it's output with the conditional in disk_initate_seek
        if( NULL != DISKSIM_DISK_LOG )
        {
    ...
    // is it firstblkno > last ok?
                ...
                && (currblkno == (firstblkno-1)))
        {
            ...
            curr->bcount = 1;
            //            if( ((currdisk->read_direct_to_buffer || !(curr->flags & READ)) && ((currdiskreq == seg->recyclereq) || !(disk_buffer_block_available(currdisk, seg, lbn)))) || (currcause < 0) )
            //            if( ((currdisk->read_direct_to_buffer || !(curr->flags & READ)) && ((currdiskreq == seg->recyclereq) || !(disk_buffer_block_available(currdisk, seg, lbn)))) )
            //            {
            //                curr->bcount = 2;
            //            }
        }
        ...
        double maxtime)
{
    ...
    tmpblkno = curr->bcount;  // TODO need to investigate this...appears to be an override
    tmptime = curr->time;
    ...
    if (curr->time < maxtime) {
        ...
        currdisk->model->layout->dm_get_track_boundaries(currdisk->model, &destpbn, (LBA_t)0, &lastontrack, (LBA_t)0);
        ...
        curr->bcount = tmpblkno;  // TODO need to investigate this...appears to be an override
        if ((!(curr->flags & READ))
                && (tmptime < currdisk->minimum_seek_delay)) {
            tmptime = currdisk->minimum_seek_delay;
        }
    }
    //    else { // hurst_r  don't know why we are adding 1 to this variable
    //        tmptime = maxtime + 1.0;
    //    }

#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_buffer_estimate_servtime  servtime %f\n", simtime, tmptime );
    fflush(outputfile);
#endif
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_get_servtime  devno %d, blkno %lld, bcount %d, flags %x\n", simtime, req->devno, req->blkno, req->bcount, req->flags );
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_get_seektime  devno %d, blkno %lld, bcount %d, flags %x\n", simtime, req->devno, req->blkno, req->bcount, req->flags );
...
#ifdef DEBUG_DISKCTLR
    fprintf (outputfile, "********** %f: disk_get_acctime  devno %d, blkno %lld, bcount %d, flags %x\n", simtime, req->devno, req->blkno, req->bcount, req->flags );
