...
event * pf_io_done_notify (ioreq_event *curr, void *ctx)
{
   ...
   if (pf_printhack) {
     fprintf (outputfile, "pf_io_done_notify: curr->buf %p, curr->opid %x, curr->blkno %lld\n", curr->buf, curr->opid, curr->blkno);
   }
...
#ifdef DEBUG_PFSIM
   fprintf (outputfile, "*** %f: pf_handle_ioreq_event IOREQ cpu=%d opid=%d buf=%p blkno=%lld flags=%x, bcount=%d\n", simtime, cpu_ev->cpunum, curr->opid, curr->buf, curr->blkno, curr->flags, curr->bcount);
#endif
...
#ifdef DEBUG_PFSIM
//   if (pf_printhack)
   fprintf (outputfile, "*** %f: pf_handle_ioacc_event IOACC cpu=%d opid=%d blkno=%lld\n", simtime, cpu_ev->cpunum, curr->opid, curr->blkno);
#endif
...
#ifdef DEBUG_PFSIM
//	if (pf_printhack)
   fprintf (outputfile, "*** %f: pf_handle_io_internal_event IO INTERNAL cpu=%d type %d opid=%d blkno=%lld buf=%p\n", simtime, cpu_ev->cpunum, curr->type, curr->opid, curr->blkno, curr->buf);
#endif
