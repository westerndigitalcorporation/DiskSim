...
#ifdef DEBUG_INTQ
    fprintf( outputfile, "******** %f: Entered addtointq  event %p, time %f, type %s (%d)\n", simtime, newint, newint->time, getEventString(newint->type), newint->type );
#endif
...
void disksim_printstats2()
{
    ...
    fprintf(outputfile, "Memory Leak (bytes)       : %d\n", (numEventBlocksAllocated - disksim->extraqlen) * sizeof( struct ev ) );
...
void disksim_simulate_event (int num)
{
    ...
    if ((curr = getnextevent()) == NULL) {
        ...
        if( NULL != DISKSIM_EVENT_LOG )
        {
            ...
            {
                // output data header
                fprintf( DISKSIM_EVENT_LOG, "time,typeString,typeValue,current,next,prev\n" );
            }
