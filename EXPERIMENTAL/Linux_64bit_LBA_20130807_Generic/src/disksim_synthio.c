...
void dump_synthio_generator( synthio_generator *gen )
{
    if( NULL != outputfile )
    {
        ...
        fprintf( outputfile, "\n  devno        : %d", *(gen->devno) );
        fprintf( outputfile, "\n  numblocks    : %lld", gen->numblocks );
        fprintf( outputfile, "\n  sectsperdisk : %lld", gen->sectsperdisk );
        fprintf( outputfile, "\n  blksperdisk  : %lld", gen->blksperdisk );
...
#ifdef DEBUG_SYNTHIO
    fprintf (outputfile, "*** %f: synthio_appendio New request %d, timeinterval %f, devno %d, blkno %lld, bcount %d, flags %x\n", simtime, SYNTHIO_IOCNT, new_event->time, new_event->devno, new_event->blkno, new_event->bcount, new_event->flags);
#endif

    if( NULL != DISKSIM_SYNTHIO_LOG )
    {
        fprintf( DISKSIM_SYNTHIO_LOG, "%f %d %lld %d %d (%x)\n", new_event->time, new_event->devno, new_event->blkno, new_event->bcount, new_event->flags, new_event->flags );
...
void synthio_initialize_generator (process *procp)
{
    ...
    fprintf (outputfile, "****** %f:   timeinterval %f, cause %d, devno: %d, blkno: %lld, bcount: %d, flags: %d\n",  simtime, tmp->time, tmp->cause, tmp->devno, tmp->blkno, tmp->bcount, tmp->flags );
...
static int synthio_generatenextio (synthio_generator *gen)
{
    ...
    fprintf (outputfile, "*** %f: synthio_generatenextio   SynthIO used, timeinterval %f, cause %d, devno: %d, blkno: %lld, bcount: %d, flags: %d\n",  simtime, tmp->time, tmp->cause, tmp->devno, tmp->blkno, tmp->bcount, tmp->flags );
    fprintf (outputfile, "*** %f: synthio_generatenextio   SynthIO used, number %d, blksperhead %lld, probseq %lf, probloc %lf, probread %lf\n",  simtime, gen->number, gen->blksperdisk, gen->probseq, gen->probloc, gen->probread );
    fflush( outputfile );
#endif
...
#ifdef DEBUG_SYNTHIO
    fprintf (outputfile, "*** %f: synthio_generatenextio   SynthIO used, time interval: %lf, cause: %d, blkno: %lld, bcount: %d, flags: %d\n",  simtime, tmp->time, tmp->cause, tmp->blkno, tmp->bcount, tmp->flags );
    fflush( outputfile );
#endif
