...
#ifdef DEBUG_BUS
    fprintf( outputfile, "*** %f: bus_delay  adds this event to delayed_event  type %d, devno %d, blkno %lld, bcount %d, flags 0x%x, delay %f msec\n", simtime, curr->type, curr->devno, curr->blkno, curr->bcount, curr->flags, delay );
#endif
...
#ifdef DEBUG_BUS
   fprintf (outputfile, "*** %f: bus_deliver_event  busno %d, slotno %d, busdevno %d, busdevtype %d, type %s (%d), devno %d, blkno %lld, bcount %d, flags 0x%x\n", simtime, busno, slotno, devno, devtype, getEventString(curr->type), curr->type, curr->devno, curr->blkno, curr->bcount, curr->flags );
#endif
...
int load_bus_topo(struct lp_topospec *t, int *parentctlno) {
  ...
  for(c = 0; c < t->l->values_len; c++) {
    ...
    if(t->l->values[c]->t != TYPE_TOPOSPEC) {
