...
LBA_t simpledisk_get_number_of_blocks (int devno)
{
...
void simpledisk_get_mapping (int maptype, int devno, LBA_t blkno, int *cylptr, int *surfaceptr, int *blkptr)
{
   simpledisk_t *currdisk = getsimpledisk (devno);

   if ((blkno < 0) || (blkno >= currdisk->numblocks)) {
      fprintf(stderr, "Invalid blkno at simpledisk_get_mapping: %lld\n", blkno);
      exit(1);
   }
