...
typedef struct seg {
   ...
   int          outbcount;
   LBA_t        minreadaheadblkno;      /* min prefetch blkno + 1 */
   LBA_t        maxreadaheadblkno;      /* max prefetch blkno + 1 */

   struct diskreq_t *diskreqlist;       /* sorted by ascendingly first blkno */
   int              size;
       /* diskreqlist normally starts as a single request, more can be added by read hits
        * on the segment and write
        * combining
        */

   ioreq_event *access;                 /* copy of the active ioreq using this seg -rcohen */
   LBA_t        hold_blkno;             /* used for prepending */
...
typedef struct disk {
  ...
  LBA_t track_low, track_high; // track boundaries for the current track
                               // get lowest lbn and highest lbn
...
int disk_get_distance(
        int diskno,
        ioreq_event *req,
        int exact,
        int direction);

double disk_get_servtime(
        int diskno,
        ioreq_event *req,
        int checkcache,
        double maxtime);

double disk_get_seektime(
        int diskno,
        ioreq_event *req,
        int checkcache,
        double maxtime);

double disk_get_acctime(
        int diskno,
        ioreq_event *req,
        double maxtime);
...
void disk_bus_delay_complete(
        int devno,
        ioreq_event *curr,
        int sentbusno);

void disk_bus_ownership_grant(
        int devno,
        ioreq_event *curr,
        int busno,
        double arbdelay);
