...
char *simResultString = "diskCtlrTimeBegin,diskCtlrTimeEnd,diskCtlrQueueDepth,diskCtlrQueueDepthEnd,tagID_begin,tagID_end,diskCacheHitStatus,CCT,qCCT"
                        ...
                        ",SeekDistance[1],SeekTime[1],Latency[1],XferTime[1],AccessTime[1],rpo_calc[1],BlocksXferred[1],block_first[1],block_last[1]"
                        ",tagID[2],opid[2],devno[2],blkno[2],blkcnt[2],flags[2]"
                        ",cylBegin[2],headBegin[2],thetaBegin[2],cylDest[2],headDest[2],sectorDest[2]"
                        ",SeekDistance[2],SeekTime[2],Latency[2],XferTime[2],AccessTime[2],rpo_calc[2],BlocksXferred[2],block_first[2],block_last[2]";
...
void simresult_dumpSimResult( FILE *iob, int index )
{
    ...
    {
        fprintf( iob, ",%d,%d,%d,%d,%d,%u,%d,%d,%u,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d",
                ...
                DISKSIM_SIM_RESULT_LIST[index].seekResults[i].rpo_calc,
...
void simresult_update_rpo_calc( int tagID, double rpo_calc )
{
    int seekResultIndex = DISKSIM_SIM_RESULT_LIST[tagID - 1].seekResultIndex;

    DISKSIM_SIM_RESULT_LIST[tagID - 1].seekResults[seekResultIndex].rpo_calc = rpo_calc;
}
