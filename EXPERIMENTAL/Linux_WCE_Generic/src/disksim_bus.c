...
void bus_event_arrive(ioreq_event *ptr)
{
   ...
   {
        dumpIOReq( "Entered bus_event_arrive: ioreq_event  ", curr->delayed );
   }
#endif
...
#ifdef DEBUG_BUS
   fprintf (outputfile, "*** %f: bus_deliver_event  busno %d, slotno %d, busdevno %d, busdevtype %d, type %s (%d), devno %d, blkno %d, bcount %d, flags 0x%x\n", simtime, busno, slotno, devno, devtype, getEventString(curr->type), curr->type, curr->devno, curr->blkno, curr->bcount, curr->flags );
#endif
