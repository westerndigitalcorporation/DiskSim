...
void dump_disk_buffer_seqments ( FILE *fp, segment *seg, const char *msg )
{
    int i;

    fprintf( fp, "\ndump_disk_buffer_seqments (%s): %p", msg, seg );
    fprintf( fp, "\nindex,seg,next,prev,time,state,startblkno,endblkno,outstate,outbcount,minreadaheadblkno,maxreadaheadblkno,size,hold_blkno,hold_bcount,access_next,access_prev,access_tagID,access_devno,access_blkno,access_bcount,access_flags" );
        ...
        if (currdiskreq->hittype == BUFFER_COLLISION) {
		  ...
		  if (!disk_buffer_reusable_segment_check(currdisk, seg)) {
            ...
            if ((first_ioreq->blkno == (last_ioreq->blkno + last_ioreq->bcount))
                    ...
                    && (seg->size
                            > (first_ioreq->blkno
                                    - (disk_buffer_seg_owner(seg, FALSE))->inblkno))) {
...
// Initializes current segment in currdiskreq ?? hurst_r
int disk_buffer_attempt_seg_ownership (disk *currdisk, diskreq *currdiskreq)
{
    ...
    if (!tmp_diskreq) {
        ...
        seg->access->tagID = currdiskreq->ioreqlist->tagID;
